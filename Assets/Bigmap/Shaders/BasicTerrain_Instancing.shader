Shader "BigMap/BasicTerrain_Instancing" {
	Properties {
	...
	SubShader {
		...
		//Only need to Props the _NeighbourVector and _ChunkRootPosition
		UNITY_INSTANCING_CBUFFER_START(Props)
			UNITY_DEFINE_INSTANCED_PROP(float4, _NeighbourVector)
			UNITY_DEFINE_INSTANCED_PROP(float3, _ChunkRootPosition)
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			//Get the wposition;
			float3 chunkRootPosition = UNITY_ACCESS_INSTANCED_PROP(_ChunkRootPosition);
			float3 wpos = v.vertex.xyz + chunkRootPosition;
			...
			float4 neighbourVectorf = UNITY_ACCESS_INSTANCED_PROP(_NeighbourVector);
			int4 neighbourVector = int4(int(neighbourVectorf.x), int(neighbourVectorf.y), int(neighbourVectorf.z), int(neighbourVectorf.w));
			...
			{
				offsetFactor=min(max(0.0, (length(_CameraPos.xz - wpos.xz) - _LowerBound) / (_UpperBound - _LowerBound)), 1.0);
			}
			...
			float tm = _TotalResolution * _Meter;
			...
			height /= 3.0;
			//wpos.y += height * _HeightFactor;
			wpos +=  float3(0,height*_HeightFactor,0);
			v.vertex.xyz = wpos - chunkRootPosition;
			//v.vertex.xyz = mul(unity_WorldToObject, float4(wpos,1.0));
			float2 tex = wpos.zx / _DetailMapRepeatSize;

			o.wuv = wuv;
			//o.worldPos = wpos;
			o.uv_DetailTex = wpos.zx / _DetailMapRepeatSize;
		}
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			//float4 detail_tex1 = tex2D(_DetailTex1, IN.uv_DetailTex);
			//float4 detail_tex2 = tex2D(_DetailTex2, IN.uv_DetailTex);
			//float4 detail_tex3 = tex2D(_DetailTex3, IN.uv_DetailTex);
			//float4 detail_tex4 = tex2D(_DetailTex4, IN.uv_DetailTex);
